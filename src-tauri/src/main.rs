...
enum AppError {
    ...
    #[error("JSON serialization/deserialization error: {0}")] Json(#[from] serde_json::Error),
    #[error("Tauri path resolution error: {0}")] TauriPath(String),
    #[error("Configuration error: {0}")] Config(String),
    #[error("Mod operation failed: {0}")] ModOperation(String),
    #[error("Resource not found: {0}")] NotFound(String),
    #[error("Operation cancelled by user")]
    UserCancelled,
    #[error("Shell command failed: {0}")] ShellCommand(String),
    #[error("Zip error: {0}")] Zip(#[from] zip::result::ZipError),
    #[error("7z error: {0}")] SevenZ(#[from] sevenz_rust::Error),
    #[error("RAR error: {0}")] Rar(#[from] unrar::error::UnrarError),
    #[error("Unsupported archive type: {0}")] UnsupportedArchive(String),
}
...
lazy_static! {
    static ref MOD_NAME_CLEANUP_REGEX: Regex = Regex::new(
        r"(?i)(_v\d+(\.\d+)*|_DISABLED|DISABLED_|\(disabled\)|^DISABLED_)"
    ).unwrap();
    static ref CHARACTER_NAME_REGEX: Regex = Regex::new(
        r"(?i)(Raiden|Shogun|HuTao|Tao|Zhongli|Ganyu|Ayaka|Kazuha|Yelan|Eula|Klee|Nahida)"
    ).unwrap();
    ...
    static ref NAME_CLEANUP_REGEX: Regex = Regex::new(
        r"(?i)[_\-.\s]+|(_v\d+(\.\d+)*)|(_af)|(_nsfw)|(\(disabled\))|(\(.*\))|(\[.*\])|(^DISABLED_)"
    ).unwrap();
...
struct Entity {
    ...
    base_image: Option<String>,
    mod_count: i32,
    enabled_mod_count: Option<i32>,
    recent_mod_count: Option<i32>,
    favorite_mod_count: Option<i32>,
}
#[derive(Serialize, Deserialize, Debug, Clone)]
struct Asset {
    id: i64,
...
fn run_traveler_migration_logic(
    ...
    let base_mods_path = get_mods_base_path_from_settings(db_state).map_err(|e|
        format!("[Migration] Failed to get mods base path: {}", e)
    )?;
    ...
    let migration_status = get_setting_value(
        conn,
        SETTINGS_KEY_TRAVELER_MIGRATION_COMPLETE
    ...
    let traveler_info: Option<(i64, String)> = conn
        .query_row("SELECT id, slug FROM entities WHERE slug = 'traveler'", [], |row|
            Ok((row.get(0)?, row.get(1)?))
        )
        .optional()
        .map_err(|e| format!("[Migration] DB Error fetching Traveler info: {}", e))?;

    if traveler_info.is_none() {
        let msg =
            "[Migration] Traveler entity not found. Migration not needed or already partially done.";
        println!("{}", msg);
        ...
        conn
            ...
            .map_err(|e|
                format!("[Migration] Failed to mark as complete after Traveler not found: {}", e)
            )?;
        return Ok(msg.to_string());
    }
    ...
    let aether_info: Option<(i64, String, String)> = conn
        .query_row(
        ...
        .optional()
        .map_err(|e| format!("[Migration] DB Error fetching Aether info: {}", e))?;

    // Fetch Lumine info (ID, Category Slug)
    let lumine_info: Option<(i64, String, String)> = conn
        .query_row(
        ...
        .optional()
        .map_err(|e| format!("[Migration] DB Error fetching Lumine info: {}", e))?;

    if aether_info.is_none() || lumine_info.is_none() {
        let msg =
            "[Migration] Aether or Lumine entity not found. Cannot perform migration. Ensure definitions are loaded.";
        println!("{}", msg);
    ...
    if aether_cat_slug != lumine_cat_slug {
        println!(
            "[Migration] Warning: Aether ({}) and Lumine ({}) appear to be in different categories. Using Aether's category for path construction.",
            aether_cat_slug,
            lumine_cat_slug
        );
    ...
    let mut assets_to_migrate = Vec::<(i64, String, String)>::new(); // (id, name, folder_name)
    {
        // Scope for statement
        let mut stmt = conn
            .prepare("SELECT id, name, folder_name FROM assets WHERE entity_id = ?1")
            .map_err(|e| format!("[Migration] Failed to prepare asset fetch statement: {}", e))?;
        let rows = stmt
            .query_map(params![traveler_id], |row|
        ...
        for row_result in rows {
            match row_result {
                ...
                Err(e) => {
                    return Err(format!("[Migration] Error reading asset row: {}", e));
                }
            }
        }
    }
    ...
    let maps = fetch_deduction_maps(conn).map_err(|e|
        format!("[Migration] Failed to fetch deduction maps: {}", e)
    )?;

    // --- Start Transaction ---
    let tx = conn
        .transaction()
        .map_err(|e| format!("[Migration] Failed to start transaction: {}", e))?;
    ...
    for (asset_id, asset_name, current_clean_relative_path) in assets_to_migrate {
        println!(
            "[Migration] Processing Asset ID: {}, Name: '{}', Current DB Path: '{}'",
            asset_id,
            asset_name,
            current_clean_relative_path
        );
        ...
        let current_folder_name = current_relative_path_buf
            .file_name()
            .unwrap_or_default()
            .to_string_lossy();
        ...
        if !current_folder_name.is_empty() {
            if let Some(hinted_slug) = find_entity_slug_from_hint(&current_folder_name, &maps) {
                ...
                current_folder_name.to_lowercase().contains("lumine") ||
                current_folder_name.to_lowercase().contains("female")
            {
                target_id = lumine_id;
                target_slug = lumine_slug.clone();
                target_reason = "Folder name keyword";
            } else if
                current_folder_name.to_lowercase().contains("aether") ||
                current_folder_name.to_lowercase().contains("male")
            {
                target_id = aether_id;
                target_slug = aether_slug.clone();
                target_reason = "Folder name keyword";
        ...
        println!(
            "[Migration]   -> Assigning to {} (ID: {}) based on: {}",
            target_slug,
            target_id,
            target_reason
        );
        ...
        // --- Calculate Paths ---
        let mod_folder_base_name_from_db = current_relative_path_buf
            .file_name()
            .unwrap_or_default()
            .to_string_lossy();
        if mod_folder_base_name_from_db.is_empty() {
            let err = format!(
                "[Migration]   -> ERROR: Cannot extract base name from DB path '{}'. Skipping asset {}.",
                current_clean_relative_path,
                asset_id
            );
            println!("{}", err);
        ...
        let new_clean_relative_path_buf = PathBuf::new()
            .join(&target_category_slug)
            .join(&target_slug)
            .join(mod_folder_base_name_from_db.as_ref());
        let new_clean_relative_path_str = new_clean_relative_path_buf
            .to_string_lossy()
            .replace("\\", "/");

        // Determine the current *actual* path on disk (check enabled/disabled)
        let disabled_filename_current = format!(
            "{}{}",
            DISABLED_PREFIX,
            mod_folder_base_name_from_db
        );
        ...
        let full_path_if_disabled_current = match relative_parent_path_current {
            Some(parent) if parent.as_os_str().len() > 0 =>
                base_mods_path.join(parent).join(&disabled_filename_current),
            _ => base_mods_path.join(&disabled_filename_current),
        };

        let (current_actual_path_on_disk, is_currently_disabled) = if
            full_path_if_enabled_current.is_dir()
        {
            ...
            let err = format!(
                "[Migration]   -> ERROR: Source folder not found on disk for asset {} at '{}' or '{}'. Skipping.",
                asset_id,
                full_path_if_enabled_current.display(),
                full_path_if_disabled_current.display()
            );
            println!("{}", err);
            errors.push(err);
        ...
        println!(
            "[Migration]   -> Current path on disk: '{}' (Disabled: {})",
            current_actual_path_on_disk.display(),
        ...
        let new_actual_dest_path_on_disk = base_mods_path
            .join(&target_category_slug)
            .join(&target_slug)
            .join(&new_folder_name_on_disk);
        println!(
            "[Migration]   -> New destination path on disk: '{}'",
            new_actual_dest_path_on_disk.display()
        );

        // --- Perform Filesystem Move (before DB commit, but after tx start) ---
        if current_actual_path_on_disk == new_actual_dest_path_on_disk {
            println!(
                "[Migration]   -> Skipping filesystem move (paths are the same). Might indicate only DB update needed."
            );
        } else {
            ...
            if let Some(parent) = new_actual_dest_path_on_disk.parent() {
                if !parent.exists() {
                    ...
                    if let Err(e) = fs::create_dir_all(parent) {
                        let err = format!(
                            "[Migration]   -> ERROR: Failed to create parent directory '{}': {}. Skipping asset {}.",
                            parent.display(),
                            e,
                            asset_id
                        );
                ...
                let err = format!(
                    "[Migration]   -> ERROR: Cannot determine parent directory for new path '{}'. Skipping asset {}.",
                    new_actual_dest_path_on_disk.display(),
                    asset_id
                );
                println!("{}", err);
            ...
            if new_actual_dest_path_on_disk.exists() {
                let err = format!(
                    "[Migration]   -> ERROR: Target path '{}' already exists. Skipping asset {}.",
                    new_actual_dest_path_on_disk.display(),
                    asset_id
                );
                println!("{}", err);
            ...
            if let Err(e) = fs::rename(&current_actual_path_on_disk, &new_actual_dest_path_on_disk) {
                let err = format!(
                    "[Migration]   -> ERROR: Failed to move folder for asset {}: {}. Skipping.",
                    asset_id,
                    e
                );
                println!("{}", err);
        ...
        println!(
            "[Migration]   -> Updating DB: asset_id={}, new_entity_id={}, new_folder_name='{}'",
            asset_id,
            target_id,
            new_clean_relative_path_str
        );
        let changes = tx
            .execute(
    ...
    if errors.is_empty() {
        ...
        let deleted_entity_count = tx
            .execute("DELETE FROM entities WHERE id = ?1", params![traveler_id])
        ...
        if deleted_entity_count > 0 {
            ...
            println!(
                "[Migration] Traveler entity already deleted or delete failed (0 rows affected)."
            );
        }
        ...
        let final_msg =
            format!("Traveler migration completed successfully. Migrated {} assets.", migrated_count);
        println!("[Migration] {}", final_msg);
        Ok(final_msg)
    } else {
        // --- Rollback Transaction due to errors ---
        let err_summary = format!(
            "[Migration] Migration failed with {} error(s). Rolling back changes.",
            errors.len()
        );
        println!("{}", err_summary);
        for e in &errors {
...
fn find_entity_slug_from_hint(hint: &str, maps: &DeductionMaps) -> Option<String> {
    ...
    if let Some(first_word_cleaned) = cleaned_hint.split_whitespace().next() {
        if first_word_cleaned.len() > 1 {
            // Avoid matching single letters
    ...
    for (entity_name_first, entity_slug) in &maps.lowercase_entity_firstname_to_slug {
        if cleaned_hint.starts_with(entity_name_first) && entity_name_first.len() > 1 {
            // Avoid matching 'a' etc.
    ...
    if cleaned_hint.len() > 3 {
        for (entity_name_lower, entity_slug) in &maps.lowercase_entity_name_to_slug {
            if entity_name_lower.contains(&cleaned_hint) {
                println!(
                    "[find_entity_slug]   -> Match via P11: known name contains cleaned hint."
                );
                return Some(entity_slug.clone());
            }
        }
    ...
    if cleaned_hint.len() > 3 {
        for (entity_name_first_two, entity_slug) in &maps.lowercase_entity_first_two_words_to_slug {
            if entity_name_first_two.contains(&cleaned_hint) {
                println!(
                    "[find_entity_slug]   -> Match via P12: known first two words contains cleaned hint."
                );
                return Some(entity_slug.clone());
            }
        }
    ...
    if cleaned_hint.len() > 2 {
        // Can be shorter here maybe
        for (entity_name_first, entity_slug) in &maps.lowercase_entity_firstname_to_slug {
            if entity_name_first.contains(&cleaned_hint) {
                println!(
                    "[find_entity_slug]   -> Match via P13: known first name contains cleaned hint."
                );
                return Some(entity_slug.clone());
            }
        }
...
fn get_internal_db_slug(db_path: &PathBuf) -> Result<Option<String>, AppError> {
    ...
    let conn = Connection::open_with_flags(db_path, flags).or_else(|e| {
...
fn find_asset_ini_paths(
    ...
    let filename_osstr = relative_path_buf
        .file_name()
        .ok_or_else(||
            AppError::ModOperation(
                format!(
                    "Could not extract filename from DB path: {}",
                    asset_info.clean_relative_path
                )
            )
        )?;
    ...
    if filename_str.is_empty() {
        println!(
            "[find_asset_ini_paths] ERROR: Filename extracted from DB path is empty: {}",
            asset_info.clean_relative_path
        );
    ...
    let full_path_if_disabled = match relative_parent_path {
        Some(parent) if parent.as_os_str().len() > 0 =>
            base_mods_path.join(parent).join(&disabled_filename),
    ...
    let mod_folder_path = if full_path_if_enabled.is_dir() {
        ...
        println!(
            "[find_asset_ini_paths] Mod folder not found for asset ID {}. Checked {} and {}",
            asset_id,
    ...
    println!(
        "[find_asset_ini_paths] Found {} INI files in {}: {:?}",
        ini_paths.len(),
...
fn deduce_mod_info_v2(
    ...
    let mod_folder_name = match mod_folder_path.file_name() {
        ...
        None => {
            eprintln!(
                "[Deduce V2] Error: Cannot get folder name from path: {}",
                mod_folder_path.display()
            );
            return None;
        }
    ...
    while let Some(path) = current_path {
        ...
        if let Some(folder_name) = path.file_name().and_then(|n| n.to_str()) {
            if let Some(slug) = find_entity_slug_from_hint(folder_name, maps) {
                ...
                println!(
                    "[Deduce V2]   -> Found entity via parent folder: '{}' -> {}",
                    folder_name,
                    found_entity_slug.as_ref().unwrap()
                );
                break;
            }
        }
        ...
        .find(
            |entry|
                ...
                entry
                    .path()
                    .extension()
                    .map_or(false, |ext| ext.eq_ignore_ascii_case("ini"))
        )
        .map(|e| e.into_path());
    ...
    if let Some(ini_path) = ini_path_option {
        ...
        if let Ok(ini_content) = fs::read_to_string(&ini_path) {
            if let Ok(ini) = Ini::load_from_str(&ini_content) {
                for section_name in ["Mod", "Settings", "Info", "General"] {
                    if let Some(section) = ini.section(Some(section_name)) {
                        ...
                        if let Some(name) = section.get("Name").or_else(|| section.get("ModName")) {
                            info.mod_name = name.trim().to_string();
                        }
                        if let Some(author) = section.get("Author") {
                            info.author = Some(author.trim().to_string());
                        }
                        ...
                        if
                            let Some(target) = section
                                .get("Target")
                                .or_else(|| section.get("Entity"))
                                .or_else(|| section.get("Character"))
                        {
                            ini_target_hint = Some(target.trim().to_string());
                        }
                        if let Some(typ) = section.get("Type").or_else(|| section.get("Category")) {
                            info.mod_type_tag = Some(typ.trim().to_string());
                            ini_type_hint = info.mod_type_tag.clone();
                        } // Store type hint
                    }
                }
                println!(
                    "[Deduce V2] INI parsed. Name='{}', Author='{:?}', TargetHint='{:?}', TypeHint='{:?}'",
                    info.mod_name,
                    info.author,
                    ini_target_hint,
                    ini_type_hint
                ...
                eprintln!(
                    "[Deduce V2] Warning: Failed to parse INI content from {}",
                    ini_path.display()
                );
            }
        } else {
            eprintln!(
                "[Deduce V2] Warning: Failed to read INI file content from {}",
                ini_path.display()
            );
        }
    } else {
    ...
    if found_entity_slug.is_none() {
        if let Some(target_hint) = &ini_target_hint {
            ...
            if let Some(slug) = find_entity_slug_from_hint(target_hint, maps) {
                found_entity_slug = Some(slug);
                println!(
                    "[Deduce V2]   -> Found entity via INI target hint: '{}' -> {}",
                    target_hint,
                    found_entity_slug.as_ref().unwrap()
                );
            }
                        ...
                        if let Some(stem) = entry.path().file_stem().and_then(OsStr::to_str) {
                            if !stem.is_empty() {
                                ...
                                if let Some(slug) = find_entity_slug_from_hint(stem, maps) {
                                    found_entity_slug = Some(slug);
                                    println!(
                                        "[Deduce V2]   -> Found entity via internal filename stem: '{}' -> {}",
                                        stem,
                                        found_entity_slug.as_ref().unwrap()
                                    );
    ...
    if found_entity_slug.is_none() {
        ...
        if let Some(slug) = find_entity_slug_from_hint(&mod_folder_name, maps) {
            found_entity_slug = Some(slug);
            println!(
                "[Deduce V2]   -> Found entity via mod folder name: '{}' -> {}",
                mod_folder_name,
                found_entity_slug.as_ref().unwrap()
            );
        }
    ...
    println!(
        "[Deduce V2] Final Assignment Logic. Found Entity Slug So Far: {:?}",
        found_entity_slug
    );
    if let Some(ref entity_slug) = found_entity_slug {
        ...
        while let Some(path) = current_path_cat {
            ...
            if let Some(folder_name) = path.file_name().and_then(|n| n.to_str()) {
                ...
                if let Some(slug) = maps.lowercase_category_name_to_slug.get(&lower_folder_name) {
                    ...
                    println!(
                        "[Deduce V2]       -> Found category via parent lowercase name: {} -> {}",
                        lower_folder_name,
                        slug
                    );
                    break; // Found best match, stop walking up
        ...
        if fallback_category_slug.is_none() {
            ...
            if let Some(type_hint) = &ini_type_hint {
                ...
                if maps.category_slug_to_id.contains_key(type_hint) {
                    ...
                    let Some(slug) = maps.lowercase_category_name_to_slug.get(&lower_type_hint)
                {
                    fallback_category_slug = Some(slug.clone());
                    println!(
                        "[Deduce V2]       -> Matched category via INI exact lowercase name: {} -> {}",
                        lower_type_hint,
                        slug
                    );
                } else {
                    ...
                    for (cat_name_lower, cat_slug) in &maps.lowercase_category_name_to_slug {
                        if cat_name_lower.starts_with(&lower_type_hint) {
                            ...
                            println!(
                                "[Deduce V2]       -> Matched category via INI name prefix: {} -> {}",
                                cat_name_lower,
                                cat_slug
                            );
                            break;
                ...
                if fallback_category_slug.is_none() {
                    for (cat_name_lower, cat_slug) in &maps.lowercase_category_name_to_slug {
                        if lower_type_hint.len() > 2 && cat_name_lower.contains(&lower_type_hint) {
                            fallback_category_slug = Some(cat_slug.clone());
                            println!(
                                "[Deduce V2]       -> Matched category via INI name contains: {} -> {}",
                                cat_name_lower,
                                cat_slug
                            );
                            break;
        ...
        if fallback_category_slug.is_none() {
            println!(
                "[Deduce V2]   Fallback Prio 3: Trying top-level folder name for CATEGORY match..."
            );
                ...
                if let Some(top_level_component) = relative_path.components().next() {
                    if let Some(top_folder_name) = top_level_component.as_os_str().to_str() {
                        ...
                        if maps.category_slug_to_id.contains_key(top_folder_name) {
                            ...
                            let Some(slug) = maps.lowercase_category_name_to_slug.get(
                                &lower_top_folder
                            )
                        {
                            fallback_category_slug = Some(slug.clone());
                            println!(
                                "[Deduce V2]       -> Matched category via top-level exact name: {} -> {}",
                                lower_top_folder,
                                slug
                            );
                        } else {
                            ...
                            for (cat_slug, _) in &maps.category_slug_to_id {
                                if
                                    cat_slug.starts_with(&lower_top_folder) ||
                                    lower_top_folder.starts_with(cat_slug)
                                {
                                    fallback_category_slug = Some(cat_slug.clone());
                            ...
                            if !fuzzy_match_found {
                                for (
                                    cat_name_lower,
                                    cat_slug,
                                ) in &maps.lowercase_category_name_to_slug {
                                    if
                                        cat_name_lower.starts_with(&lower_top_folder) ||
                                        lower_top_folder.starts_with(cat_name_lower)
                                    {
                                        fallback_category_slug = Some(cat_slug.clone());
                                        println!(
                                            "[Deduce V2]       -> Matched category via top-level fuzzy name prefix: {} -> {}",
                                            cat_name_lower,
                                            cat_slug
                                        );
                                        fuzzy_match_found = true;
                            ...
                            if !fuzzy_match_found {
                                for (
                                    ...
                                    if
                                        lower_top_folder.len() > 2 &&
                                        cat_name_lower.contains(&lower_top_folder)
                                    {
                                        fallback_category_slug = Some(cat_slug.clone());
                                        println!(
                                            "[Deduce V2]       -> Matched category via top-level fuzzy name contains: {} -> {}",
                                            cat_name_lower,
                                            cat_slug
                                        );
                                        break;
                        ...
                        println!("[Deduce V2]     Could not convert top-level OsStr to str.");
                    }
                } else {
                    println!("[Deduce V2]     Could not get top-level component.");
                }
            } else {
                println!("[Deduce V2]     Could not strip base path prefix.");
            }
        }
        ...
        if let Some(cat_slug) = fallback_category_slug {
            ...
            println!(
                "[Deduce V2] No category hint found, assigning hardcoded fallback: {}",
                info.entity_slug
            );
        }
    ...
    if info.mod_name.is_empty() {
        ...
        println!(
            "[Deduce V2] Warning: Name cleanup resulted in empty string, using folder name '{}'",
            info.mod_name
        );
...
fn get_asset_location_info(
    conn: &Connection,
    asset_id: i64
) -> Result<AssetLocationInfo, AppError> {
    conn.query_row(
        ...
        match e {
            rusqlite::Error::QueryReturnedNoRows =>
                AppError::NotFound(format!("Asset with ID {} not found", asset_id)),
            _ => AppError::Sqlite(e),
        }
    })
}
                    ...
                    if let Some(ext) = entry.path().extension() {
                        if ext.eq_ignore_ascii_case("ini") {
                            ...
                            if let Some(filename_osstr) = entry.path().file_name() {
                                let filename_lower = filename_osstr
                                    .to_string_lossy()
                                    .to_lowercase();

                                // Check if it's an excluded file (considering DISABLED_ prefix)
                                let base_filename = if
                                    filename_lower.starts_with(
                                        DISABLED_PREFIX.to_lowercase().as_str()
                                    )
                                {
                                    filename_lower.trim_start_matches(
                                        DISABLED_PREFIX.to_lowercase().as_str()
                                    )
                                } else {
...
fn get_current_asset_enabled_state(
    conn: &Connection,
    asset_id: i64,
    base_mods_path: &PathBuf
...
fn initialize_database(
    ...
    if needs_schema_setup {
        ...
        match get_internal_db_slug(&db_path) {
            Ok(Some(internal_slug)) if internal_slug != active_game_slug => {
                eprintln!(
                    "WARNING: Existing database {} contains slug '{}' but expected '{}'. Check startup logic.",
                    db_path.display(),
                    internal_slug,
                    active_game_slug
                );
            ...
            Err(e) =>
                eprintln!(
                    "Warning: Could not read internal slug from existing DB {}: {}",
                    db_path.display(),
                    e
                ),
    ...
    let definitions: Definitions = match
        app_handle.path_resolver().resolve_resource(&definition_resource_path)
    {
        Some(path) => {
            ...
            match fs::read_to_string(&path) {
                Ok(toml_content) => {
                    match toml::from_str(&toml_content) {
                        ...
                        Err(e) => {
                            eprintln!(
                                "ERROR: Failed to parse TOML from {}: {}. Using empty definitions.",
                                path.display(),
                                e
                            );
                ...
                Err(e) => {
                    eprintln!(
                        "ERROR: Failed to read definition file {}: {}. Using empty definitions.",
                        path.display(),
                        e
                    );
    ...
    println!(
        "Loaded {} categories from definitions for '{}'.",
        definitions.len(),
        active_game_slug
    );
    ...
    if !definitions.is_empty() {
        for (category_slug, category_def) in definitions.iter() {
            ...
            conn.execute(
                "INSERT OR IGNORE INTO entities (category_id, name, slug, description, details, base_image) VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
                params![
                    category_id,
                    OTHER_ENTITY_NAME,
                    other_slug,
...
// --- Utility Functions ---
fn get_app_data_dir(app_handle: &AppHandle) -> Result<PathBuf, AppError> {
    // Internal error type
    app_handle
        .path_resolver()
        .app_data_dir()
...
fn get_mods_base_path_from_settings(db_state: &DbState) -> Result<PathBuf, AppError> {
    // Internal error type
...
#[command]
fn set_setting(key: String, value: String, db_state: State<DbState>) -> CmdResult<()> {
...
#[command]
async fn select_directory() -> CmdResult<Option<PathBuf>> {
    // Removed AppHandle
    // FIX: Remove AppHandle from new(), use blocking dialog directly
    let result = dialog::blocking::FileDialogBuilder
        ::new()
...
#[command]
async fn select_file() -> CmdResult<Option<PathBuf>> {
    // Removed AppHandle
    // FIX: Use add_filter instead of dialog::Filter struct
    let result = dialog::blocking::FileDialogBuilder
        ::new() // FIX: Remove AppHandle
...
fn launch_executable_elevated(path: String) -> Result<(), String> {
    ...
    let path_wide: Vec<u16> = std::ffi::OsStr
        ::new(&path)
        .encode_wide()
        .chain(std::iter::once(0)) // Null-terminate
        .collect();
    let operation_wide: Vec<u16> = std::ffi::OsStr
        ::new("runas") // "runas" verb requests elevation
        .encode_wide()
...
#[command]
async fn launch_executable(path: String, _app_handle: AppHandle) -> CmdResult<()> {
    // app_handle might not be needed now
    ...
    match cmd {
                ...
                match event {
                    ...
                    tauri::api::process::CommandEvent::Error(e) => {
                        ...
                        if e.contains("os error 740") {
                            return Err(
                                format!("Failed to launch: The application requires administrator privileges. Try the 'Launch as Admin' button if available, or run XMM as administrator (not recommended). Original error: {}", e)
                            );
                        }
        ...
        Err(e) => {
            ...
            if e.to_string().contains("os error 740") {
                Err(
                    format!("Failed to launch: The application requires administrator privileges. Try running XMM as administrator (not recommended). Error: {}", e)
                )
            } else {
...
#[command]
fn get_category_entities(
    category_slug: String,
    db_state: State<DbState>
) -> CmdResult<Vec<Entity>> {
    let conn = db_state.0.lock().map_err(|_| "DB lock poisoned".to_string())?;
    let category_id: i64 = conn
        .query_row("SELECT id FROM categories WHERE slug = ?1", params![category_slug], |row|
            row.get(0)
        )
        .map_err(|e| {
            match e {
                rusqlite::Error::QueryReturnedNoRows =>
                    format!("Category '{}' not found", category_slug),
                _ => e.to_string(),
            }
        })?;
...
fn get_entity_details(entity_slug: String, db_state: State<DbState>) -> CmdResult<Entity> {
    ...
    let entity_info = {
        ...
        stmt
            .query_row(params![entity_slug], |row| {
                Ok(Entity {
                    id: row.get(0)?,
                    category_id: row.get(1)?,
                    name: row.get(2)?,
    ...
    let asset_folder_paths: Vec<String> = {
        ...
        let mut stmt = conn
            .prepare("SELECT folder_name FROM assets WHERE entity_id = ?1")
...
#[command]
fn get_assets_for_entity(
    entity_slug: String,
    db_state: State<DbState>,
    _app_handle: AppHandle
) -> CmdResult<Vec<Asset>> {
    let base_mods_path = get_mods_base_path_from_settings(&db_state).map_err(|e|
        format!("[get_assets_for_entity {}] Error getting base mods path: {}", entity_slug, e)
    )?;
    ...
    let entity_id: i64 = conn
        .query_row("SELECT id FROM entities WHERE slug = ?1", params![entity_slug], |row|
            row.get(0)
        )
        .map_err(|e| {
            match e {
                rusqlite::Error::QueryReturnedNoRows =>
                    format!("[get_assets_for_entity {}] Entity not found for assets lookup", entity_slug),
                _ =>
                    format!(
                        "[get_assets_for_entity {}] DB Error getting entity ID: {}",
                        entity_slug,
                        e
                    ),
            }
    ...
    let mut stmt = conn
        ...
        .map_err(|e|
            format!(
                "[get_assets_for_entity {}] DB Error preparing asset statement: {}",
                entity_slug,
                e
            )
        )?;
    ...
    match asset_rows_result {
            ...
            for (index, asset_result) in asset_iter.enumerate() {
                match asset_result {
                    Ok(mut asset_from_db) => {
                        ...
                        let filename_osstr = clean_relative_path_from_db
                            .file_name()
                            .unwrap_or_default();
                        let filename_str = filename_osstr.to_string_lossy();
                        if filename_str.is_empty() {
                        ...
                        let full_path_if_enabled = base_mods_path.join(
                            &clean_relative_path_from_db
                        );
                        ...
                        if full_path_if_enabled.is_dir() {
                            ...
                            asset_from_db.folder_name = clean_relative_path_from_db
                                .to_string_lossy()
                                .replace("\\", "/");
                        } else if full_path_if_disabled.is_dir() {
                            ...
                            let disabled_relative_path = match relative_parent_path {
                                Some(parent) if parent.as_os_str().len() > 0 =>
                                    parent.join(&disabled_filename),
                                _ => PathBuf::from(&disabled_filename),
                            };
                            asset_from_db.folder_name = disabled_relative_path
                                .to_string_lossy()
                                .replace("\\", "/");
                        } else {
                    ...
                    Err(e) => {
                        eprintln!(
                            "[get_assets_for_entity {}] Error processing asset row index {}: {}",
                            entity_slug,
                            index,
                            e
                        );
        ...
        Err(e) => {
            let err_msg = format!(
                "[get_assets_for_entity {}] DB Error preparing asset iterator: {}",
                entity_slug,
                e
            );
            return Err(err_msg);
...
fn toggle_asset_enabled(
    ...
    println!(
        "[toggle_asset_enabled] Toggling asset: ID={}, Name={}, UI Folder='{}', UI Enabled State={}",
        asset.id,
        asset.name,
        asset.folder_name,
        asset.is_enabled
    ...
    let clean_relative_path_from_db_str = {
        ...
        conn
            ...
            .map_err(|e|
                format!("Failed to get relative path from DB for asset ID {}: {}", asset.id, e)
            )?
    };
    ...
    println!(
        "[toggle_asset_enabled] Clean relative path from DB: '{}'",
        clean_relative_path_from_db.display()
    );
    ...
    if filename_str.is_empty() {
        return Err(
            format!(
                "Filename extracted from DB path is empty: {}",
                clean_relative_path_from_db.display()
            )
        );
    }
    ...
    println!(
        "[toggle_asset_enabled] Constructed enabled path check: {}",
        full_path_if_enabled.display()
    );
    println!(
        "[toggle_asset_enabled] Constructed disabled path check: {}",
        full_path_if_disabled.display()
    );
    ...
    let (current_full_path, target_full_path, new_enabled_state) = if full_path_if_enabled.is_dir() {
        ...
        println!(
            "[toggle_asset_enabled] Error: Mod folder not found on disk based on DB relative path!"
        );
    ...
    // Perform the rename
    fs
        ::rename(&current_full_path, &target_full_path)
        .map_err(|e|
            format!(
                "Failed to rename '{}' to '{}': {}",
...
#[command]
fn get_asset_image_path(asset_id: i64, db_state: State<DbState>) -> CmdResult<String> {
    // --- Data needed from DB ---
    ...
    // --- Acquire lock *only* for DB reads ---
    {
        ...
        let conn_guard = db_state.0
            .lock()
            .map_err(|_| format!("[get_asset_image_path ID: {}] DB lock poisoned", asset_id))?;
        let conn = &*conn_guard;

        // 1. Get base mods path from settings
        base_mods_path_str = get_setting_value(conn, SETTINGS_KEY_MODS_FOLDER)
            .map_err(|e|
                format!("[get_asset_image_path ID: {}] DB Error getting base path: {}", asset_id, e)
            )?
            .ok_or_else(||
                format!("[get_asset_image_path ID: {}] Mods folder path not set", asset_id)
            )?;

        // 2. Fetch asset info (clean path and image filename) using asset_id
        let (fetched_path, fetched_image_opt): (String, Option<String>) = conn
            .query_row(
            ...
            .map_err(|e| {
                match e {
                    rusqlite::Error::QueryReturnedNoRows =>
                        format!("[get_asset_image_path ID: {}] Asset not found.", asset_id),
                    _ =>
                        format!(
                            "[get_asset_image_path ID: {}] DB Error getting asset info: {}",
                            asset_id,
                            e
                        ),
                }
        ...
        image_filename = match fetched_image_opt {
            ...
            _ => {
                ...
                return Err(
                    format!("[get_asset_image_path ID: {}] Asset does not have an associated image filename.", asset_id)
                );
            }
        };
    ...
    let mod_folder_filename_osstr = clean_relative_path_buf
        .file_name()
        .ok_or_else(||
            format!(
                "[get_asset_image_path ID: {}] Cannot get folder filename from '{}'",
                asset_id,
    ...
    let full_path_if_disabled = match relative_parent_path {
        Some(parent) if parent.as_os_str().len() > 0 =>
            base_mods_path.join(parent).join(&disabled_mod_folder_filename),
    ...
    let current_mod_folder_path = if full_path_if_enabled.is_dir() {
        println!(
            "[get_asset_image_path ID: {}] Found enabled path: {}",
            asset_id,
            full_path_if_enabled.display()
        );
        full_path_if_enabled
        ...
        println!(
            "[get_asset_image_path ID: {}] Found disabled path: {}",
            asset_id,
            full_path_if_disabled.display()
        );
        full_path_if_disabled
        ...
        return Err(
            format!(
                "Mod folder for asset ID {} not found on disk (Checked '{}' and '{}').",
                asset_id,
                full_path_if_enabled.display(),
                full_path_if_disabled.display()
            )
    ...
    println!(
        "[get_asset_image_path ID: {}] Checking image file: {}",
        asset_id,
    ...
    if !image_full_path.is_file() {
        ...
        return Err(
            format!(
                "Image file '{}' not found in mod folder '{}'.",
                image_filename,
    ...
    println!(
        "[get_asset_image_path ID: {}] Success, returning path: {}",
        asset_id,
        image_full_path.display()
    );
...
#[command]
fn open_mods_folder(_app_handle: AppHandle, db_state: State<DbState>) -> CmdResult<()> {
    // Mark app_handle unused
    ...
    if !mods_path.exists() || !mods_path.is_dir() {
        ...
        eprintln!(
            "Configured mods folder does not exist or is not a directory: {}",
            mods_path.display()
        );
        return Err(
            format!(
...
async fn scan_mods_directory(db_state: State<'_, DbState>, app_handle: AppHandle) -> CmdResult<()> {
    ...
    if !base_mods_path.is_dir() {
        ...
        app_handle
            .emit_all(SCAN_ERROR_EVENT, &err_msg)
            .unwrap_or_else(|e| eprintln!("Failed to emit scan error event: {}", e));
        return Err(err_msg);
    }
    ...
    let deduction_maps = {
        let conn_guard = db_state.0.lock().map_err(|_| "DB lock poisoned".to_string())?;
        let conn = &*conn_guard;
        fetch_deduction_maps(conn).map_err(|e|
            format!("Failed to pre-fetch deduction maps: {}", e)
        )?
    };
    ...
    let potential_mod_folders_for_count: Vec<PathBuf> = WalkDir::new(&base_mods_path)
        ...
        .filter(|e| {
            ...
            has_ini_file(&path.to_path_buf()) ||
                (filename.starts_with("DISABLED") && !filename.starts_with(DISABLED_PREFIX))
        })
    ...
    app_handle
        .emit_all(SCAN_PROGRESS_EVENT, ScanProgress {
            processed: 0,
    ...
    let scan_task = async_runtime::spawn_blocking(move || {
        ...
        let conn = Connection::open(&db_path_str).map_err(|e|
            format!("Failed to open DB connection in scan task: {}", e)
        )?;
            ...
            match entry_result {
                Ok(entry) => {
                    ...
                    if is_directory && !processed_mod_paths.contains(&current_path) {
                        ...
                        let needs_rename =
                            filename_str.starts_with("DISABLED") &&
                            !filename_str.starts_with(DISABLED_PREFIX);
                        ...
                        if needs_rename {
                            let new_filename = format!(
                                "{}{}",
                                DISABLED_PREFIX,
                                filename_str.strip_prefix("DISABLED").unwrap_or(&filename_str)
                            );
                            if let Some(parent_path) = current_path.parent() {
                                let new_path = parent_path.join(&new_filename);
                                println!(
                                    "[Scan Task - Rename] Found incorrect prefix: '{}'. Renaming to '{}'",
                                ...
                                app_handle_clone
                                    ...
                                    .unwrap_or_else(|e|
                                        eprintln!("Failed to emit rename progress: {}", e)
                                    );
                                ...
                                match fs::rename(&current_path, &new_path) {
                                    ...
                                    Err(e) => {
                                        eprintln!(
                                            "[Scan Task - Rename] ERROR: Failed to rename folder '{}': {}. Skipping folder.",
                                            current_path.display(),
                                            e
                                        );
                                        errors_count += 1;
                                ...
                                eprintln!(
                                    "[Scan Task - Rename] ERROR: Cannot get parent path for '{}'. Skipping rename and folder.",
                                    current_path.display()
                                );
                                errors_count += 1;
                        ...
                        if has_ini_file(&current_path_for_processing) {
                            ...
                            let folder_name_only = current_path_for_processing
                                .file_name()
                                .unwrap_or_default()
                                .to_string_lossy();
                            ...
                            match
                                deduce_mod_info_v2(
                                    ...
                                    &base_mods_path_clone,
                                    &maps_clone
                                )
                            {
                                Some(deduced) => {
                                    println!(
                                        "[Scan Task] Deduced slug for '{}': {}",
                                        path_display,
                                        deduced.entity_slug
                                    );
                                    let target_entity_id_result: Option<i64> =
                                        maps_clone.entity_slug_to_id
                                            .get(&deduced.entity_slug)
                                            .copied();

                                    if let Some(target_entity_id) = target_entity_id_result {
                                        println!(
                                            "[Scan Task] Found entity ID {} for slug '{}'",
                                            target_entity_id,
                                            deduced.entity_slug
                                        );

                                        let relative_path_buf = match
                                            current_path_for_processing.strip_prefix(
                                                &base_mods_path_clone
                                            )
                                        {
                                            Ok(p) => p.to_path_buf(),
                                        ...
                                        let clean_filename = filename_str
                                        ...
                                        let existing_db_asset_id: Option<i64> = conn
                                            .query_row(
                                            ...
                                            .map_err(|e|
                                                format!(
                                                    "DB error checking for existing asset '{}': {}",
                                        ...
                                        if let Some(asset_id) = existing_db_asset_id {
                                            println!(
                                                "[Scan Task] Asset already in DB (ID: {}), path '{}'. Marking as found.",
                                                asset_id,
                                                relative_path_to_store
                                            );
                                            ...
                                            println!(
                                                "[Scan Task] Inserting new asset: EntityID={}, Name='{}', Path='{}'",
                                                target_entity_id,
                                                deduced.mod_name,
                                                relative_path_to_store
                                            );
                                                ...
                                                Err(e) => {
                                                    if
                                                        e
                                                            .to_string()
                                                            .contains(
                                                                "UNIQUE constraint failed: assets.folder_name"
                                                            )
                                                    {
                                                        ...
                                                        eprintln!(
                                                            "[Scan Task]   -> DB error inserting new asset '{}': {}",
                                                            relative_path_to_store,
                                                            e
                                                        );
                                                        errors_count += 1;
                                        ...
                                        eprintln!(
                                            "[Scan Task] CRITICAL ERROR: Deduced slug '{}' for path '{}' does NOT exist in the entity map! Skipping mod. Check DB initialization and deduction logic.",
                                            deduced.entity_slug,
                                            path_display
                                        );
                                        errors_count += 1;
        ...
        if !mods_to_prune_ids.is_empty() {
            ...
            if !ids_to_delete_sql.is_empty() {
                let placeholders = ids_to_delete_sql
                    .iter()
                    .map(|_| "?")
                    .collect::<Vec<_>>()
                    .join(",");
                ...
                app_handle_clone
                    .emit_all(
                        PRUNING_PROGRESS_EVENT,
                        format!("Deleting {} entries...", ids_to_delete_sql.len())
                    )
                    .ok();

                let delete_result = conn
                    .execute(&sql, rusqlite::params_from_iter(ids_to_delete_sql))
    ...
    match scan_task.await {
        Ok(Ok((processed, added, _updated, errors, pruned, renamed))) => {
            // Add renamed here
            let rename_msg = if renamed > 0 {
                format!(" Renamed {} incorrectly prefixed folders.", renamed)
            } else {
                "".to_string()
            };
            let summary = format!(
            ...
            println!("{}", summary);
            app_handle
                .emit_all(SCAN_COMPLETE_EVENT, summary.clone())
                .unwrap_or_else(|e| eprintln!("Failed to emit scan complete event: {}", e));
            Ok(())
        }
        Ok(Err(e)) => {
            eprintln!("Scan task failed internally: {}", e);
            app_handle
                .emit_all(SCAN_ERROR_EVENT, e.clone())
                .unwrap_or_else(|e| eprintln!("Failed to emit scan error event: {}", e));
            Err(e)
        }
        Err(e) => {
            ...
            eprintln!("{}", err_msg);
            app_handle
                .emit_all(SCAN_ERROR_EVENT, err_msg.clone())
                .unwrap_or_else(|e| eprintln!("Failed to emit scan error event: {}", e));
            Err(err_msg)
        }
    }
...
fn get_total_asset_count(db_state: State<DbState>) -> CmdResult<i64> {
    let conn = db_state.0.lock().map_err(|_| "DB lock poisoned".to_string())?;
    conn.query_row("SELECT COUNT(*) FROM assets", [], |row| row.get(0)).map_err(|e| e.to_string())
}
...
fn update_asset_info(
    ...
    // Returns Result<(), String>
    println!(
        "[update_asset_info] Start for asset ID: {}. Relocate to: {:?}. Image Data Provided: {}",
        asset_id,
        new_target_entity_slug,
        image_data.is_some()
    );
    ...
    let current_info = get_asset_location_info(conn, asset_id).map_err(|e|
        format!("Failed get current asset info: {}", e)
    )?;
    println!("[update_asset_info] Current Info: {:?}", current_info);

    // --- 2. Relocation Logic ---
    let needs_relocation =
        new_target_entity_slug.is_some() &&
        new_target_entity_slug.as_deref() != Some(&current_info.entity_slug);
    ...
    if needs_relocation {
        ...
        let (new_entity_id, new_category_slug): (i64, String) = conn
            .query_row(
        ...
        let current_filename_osstr = current_relative_path_buf
            .file_name()
            .ok_or("Cannot get current filename")?;
        ...
        let current_full_path = if full_path_if_enabled.is_dir() {
            ...
            return Err(
                format!(
                    "Cannot relocate: Source folder not found at '{}' or disabled variant.",
                    full_path_if_enabled.display()
                )
            );
        };
        ...
        let new_filename_to_use_on_disk = if
            current_full_path
                .file_name()
                .map_or(false, |name| name.to_string_lossy().starts_with(DISABLED_PREFIX))
        {
            disabled_filename // Keep disabled prefix
        } else {
            mod_base_name.to_string() // Use clean name
        };
        let new_full_dest_path_on_disk = base_mods_path
            .join(&new_category_slug)
            .join(target_slug)
            .join(&new_filename_to_use_on_disk);
        println!("[update_asset_info] New relative path for DB: {}", final_relative_path_str);
        println!(
            "[update_asset_info] New full destination path on disk: {}",
            new_full_dest_path_on_disk.display()
        );
        ...
        if new_full_dest_path_on_disk.exists() {
            return Err(
                format!(
                    "Cannot relocate: Target path '{}' already exists.",
                    new_full_dest_path_on_disk.display()
                )
            );
        }
        fs::rename(&current_full_path, &new_full_dest_path_on_disk).map_err(|e| e.to_string())?; // Add map_err
        // --- END FIX 2 ---
    ...
    let mod_folder_on_disk = if let Some(relocated_path) = final_path_on_disk {
        ...
        if full_path_if_enabled.is_dir() {
            ...
            return Err(
                format!(
                    "Mod folder not found on disk at '{}' or disabled variant.",
                    full_path_if_enabled.display()
                )
            );
        }
    ...
    println!(
        "[update_asset_info] Confirmed mod path on disk for image: {}",
        mod_folder_on_disk.display()
    );
    ...
    if !mod_folder_on_disk.is_dir() {
        ...
        println!(
            "[update_asset_info] Warning: Target mod folder {} does not exist, attempting to create.",
            mod_folder_on_disk.display()
        );
    ...
    if let Some(data) = image_data {
        ...
        fs
            ::write(&target_image_path, data)
            .map_err(|e|
                format!(
                    "Failed to save pasted image data to '{}': {}",
        ...
        let Some(source_path_str) = selected_image_absolute_path
    {
        ...
        let target_image_path = mod_folder_on_disk.join(TARGET_IMAGE_FILENAME);
        fs
            ::copy(&source_path, &target_image_path)
            .map_err(|e|
                format!("Failed to copy selected image to '{}': {}", target_image_path.display(), e)
            )?;
        ...
        println!(
            "[update_asset_info] No new image data or path provided. Fetching existing filename."
        );
        // Query existing filename. Ok if it doesn't exist (returns None)
        image_filename_to_save = conn
            ...
            .optional()
            .map_err(|e| format!("DB error fetching existing image name: {}", e))?
            .flatten(); // flatten Option<Option<String>>
    }
    println!(
        "[update_asset_info] Image handling complete. Filename to save in DB: {:?}",
...
fn delete_asset(asset_id: i64, db_state: State<DbState>) -> CmdResult<()> {
    ...
    // --- 1. Get Asset Info ---
    let asset_info = get_asset_location_info(conn, asset_id).map_err(|e|
        format!("Failed to get asset info for deletion: {}", e)
    )?;
    ...
    let filename_osstr = relative_path_buf
        .file_name()
        .ok_or_else(||
            format!("Could not extract filename from DB path: {}", asset_info.clean_relative_path)
        )?;
    let filename_str = filename_osstr.to_string_lossy();
    let disabled_filename = format!("{}{}", DISABLED_PREFIX, filename_str);
    let relative_parent_path = relative_path_buf.parent();
    ...
    let path_to_delete = if full_path_if_enabled.is_dir() {
        ...
        eprintln!(
            "[delete_asset] Warning: Mod folder not found on disk for asset ID {}. Checked {} and {}. Proceeding with DB deletion.",
            asset_id,
            full_path_if_enabled.display(),
            full_path_if_disabled.display()
        );
...
fn analyze_archive(
    ...
    let maps = {
        ...
        let conn = &*conn_guard; // Dereference the guard
        fetch_deduction_maps(conn).map_err(|e|
            format!("Analyze: Failed to fetch deduction maps: {}", e)
        )?
    };
    ...
    match extension.as_deref() {
        Some("zip") => {
            ...
            let mut archive = ZipArchive::new(file).map_err(|e|
                format!("Failed to read zip archive {}: {}", file_path.display(), e)
            )?;
        ...
        Some("7z") => {
            ...
            let mut archive = sevenz_rust::SevenZReader
                ::open(&file_path_str, Password::empty())
                .map_err(|e|
                    format!("Failed to open/read 7z archive {}: {}", file_path.display(), e)
                )?;
        ...
        Some("rar") => {
            ...
            for entry_result in &mut list_archive {
                // Keep iterating with &mut
                match entry_result {
                    Ok(header) => {
                        let path_str = header.filename
                            .to_string_lossy()
                            .replace("\\", "/")
                            .to_string();
            ...
            let ini_files_to_read: Vec<(String, PathBuf)> = header_infos
                .iter()
            ...
            if !ini_files_to_read.is_empty() {
                let mut processing_archive = Archive::new(&file_path_str)
                    .open_for_processing()
                    .map_err(|e| e.to_string())?;
                ...
                loop {
                    match processing_archive.read_header().map_err(|e| e.to_string())? {
                        Some(header_state) => {
                            ...
                            let path_str = current_filename
                                .to_string_lossy()
                                .replace("\\", "/")
                                .to_string();
                            if
                                let Some(pos) = ini_files_to_read
                                    .iter()
                                    .position(|(_, fname)| fname == &current_filename)
                            {
                                match header_state.read() {
                                    Ok((bytes, next_state)) => {
                                        ...
                                        read_count += 1;
                                        if read_count == ini_files_to_read.len() {
                                            break;
                                        }
                                    }
                                    Err(e) => {
                                        return Err(
                                            format!(
                                                "Error reading content of RAR INI '{}': {}",
                                                path_str,
                                                e
                                            )
                                        );
                                ...
                                processing_archive = header_state
                                    .skip()
                                    .map_err(|e| e.to_string())?;
                            }
                        }
    ...
    println!(
        "[analyze_archive] Pass 1: Found {} entries. Found {} INI files.",
        entries.len(),
        ini_contents.len()
    );
    ...
    for (ini_index, ini_entry) in entries.iter().enumerate() {
        if !ini_entry.is_dir && ini_entry.path.to_lowercase().ends_with(".ini") {
            ...
            if let Some(parent_path_ref) = parent_path_obj {
                ...
                if parent_path_str_norm.is_empty() {
                    continue;
                }
                let found_parent = entries.iter().position(|dir_entry| {
                    ...
                    let dir_entry_path_norm = dir_entry.path
                        .strip_suffix('/')
    ...
    for root_index in likely_root_indices.iter() {
        if let Some(root_entry) = entries.get(*root_index) {
            let root_prefix = if root_entry.path.ends_with('/') {
                root_entry.path.clone()
            } else {
                format!("{}/", root_entry.path)
            };
            for candidate in preview_candidates.iter() {
                ...
                if
                    entries
                        .iter()
                        .any(|e| !e.is_dir && e.path.eq_ignore_ascii_case(&potential_preview_path))
                {
    ...
    for (index, entry) in entries.iter_mut().enumerate() {
        if likely_root_indices.contains(&index) {
            ...
            if !first_likely_root_processed {
                ...
                let root_prefix = if entry.path.ends_with('/') {
                    entry.path.clone()
                } else {
                    format!("{}/", entry.path)
                };
                ...
                if
                    let Some((_ini_path, ini_content)) = ini_contents
                        .iter()
                        .find(
                            |(p, _)|
                                p.starts_with(&root_prefix) &&
                                p.trim_start_matches(&root_prefix).find('/') == None
                        )
                {
                    ...
                    if let Ok(ini) = Ini::load_from_str(ini_content) {
                        ...
                        for section_name in ["Mod", "Settings", "Info", "General"] {
                            if let Some(section) = ini.section(Some(section_name)) {
                                // Extract Name, Author
                                let name_val = section
                                    .get("Name")
                                    .or_else(|| section.get("ModName"));
                                // Use the INI name if found, otherwise keep the initial filename guess
                                if let Some(name) = name_val {
                                    let cleaned_ini_name = MOD_NAME_CLEANUP_REGEX.replace_all(
                                        name,
                                        ""
                                    )
                                        .trim()
                                ...
                                if author_val.is_some() {
                                    deduced_author = author_val.map(String::from);
                                }

                                // Extract Raw Hints
                                let target_val = section
                                    .get("Target")
                                    .or_else(|| section.get("Entity"))
                                    .or_else(|| section.get("Character"));
                                if target_val.is_some() {
                                    extracted_target = target_val.map(|s| s.trim().to_string());
                                }
                                let type_val = section
                                    .get("Type")
                                    .or_else(|| section.get("Category"));
                                if type_val.is_some() {
                                    extracted_type = type_val.map(|s| s.trim().to_string());
                                }
                            }
                        }
                        // Log extracted hints and assign to outer scope
                        println!(
                            "[analyze_archive] INI Extracted Hints: Target='{:?}', Type='{:?}'",
                            extracted_target,
                            extracted_type
                        );
                ...
                if final_deduced_entity_slug.is_none() {
                    ...
                    if let Some(target_hint) = &raw_ini_target_found {
                        ...
                        if let Some(slug) = find_entity_slug_from_hint(target_hint, &maps) {
                            ...
                            println!(
                                "[analyze_archive]   -> Found entity via INI target hint: '{}' -> {}",
                                target_hint,
                                final_deduced_entity_slug.as_ref().unwrap()
                            );
                        }
                    } else {
                ...
                if final_deduced_category_slug.is_none() {
                    ...
                    if let Some(type_hint) = &raw_ini_type_found {
                        let lower_type_hint = type_hint.to_lowercase();
                        println!(
                            "[analyze_archive] Trying INI type hint: '{}' (lowercase: '{}')",
                            type_hint,
                            lower_type_hint
                        );
                            ...
                            )
                        {
                            ...
                            println!(
                                "[analyze_archive]   -> Matched category via INI exact lowercase name: {} -> {}",
                                lower_type_hint,
                                slug
                            );
                        } else {
                            ...
                            for (
                                ...
                                if cat_name_lower.starts_with(&lower_type_hint) {
                                    ...
                                    println!(
                                        "[analyze_archive]   -> Matched category via INI name prefix: '{}' starts with '{}' -> {}",
                                        cat_name_lower,
                                        lower_type_hint,
                                        cat_slug
                                    );
                        ...
                        if final_deduced_category_slug.is_none() {
                            for (
                                ...
                                if
                                    lower_type_hint.len() > 2 &&
                                    cat_name_lower.contains(&lower_type_hint)
                                {
                                    final_deduced_category_slug = Some(cat_slug.clone());
                                    println!(
                                        "[analyze_archive]   -> Matched category via INI name contains: '{}' contains '{}' -> {}",
                                        cat_name_lower,
                                        lower_type_hint,
                                        cat_slug
                                    );
                        ...
                        if final_deduced_category_slug.is_none() {
                            println!(
                                "[analyze_archive]   -> No category match found from INI type hint."
                            );
                        }
                    } else {
                ...
                if let Some(stem) = Path::new(filename).file_stem().and_then(OsStr::to_str) {
                    if !stem.is_empty() {
                        ...
                        if let Some(slug) = find_entity_slug_from_hint(stem, &maps) {
                            final_deduced_entity_slug = Some(slug);
                            println!(
                                "[analyze_archive]   -> Found entity via internal filename stem: '{}' -> {}",
                                stem,
                                final_deduced_entity_slug.as_ref().unwrap()
                            );
                            file_match_found = true;
    ...
    if final_deduced_entity_slug.is_none() || final_deduced_category_slug.is_none() {
        ...
        if let Some(stem) = file_path.file_stem().and_then(OsStr::to_str) {
            ...
            if final_deduced_category_slug.is_none() {
                ...
                println!(
                    "[analyze_archive] Trying archive filename stem for Category: '{}' (cleaned lowercase: '{}')",
                    stem,
                    cleaned_stem
                );
                ...
                if maps.category_slug_to_id.contains_key(stem) {
                    ...
                    let Some(slug) = maps.lowercase_category_name_to_slug.get(&cleaned_stem)
                {
                    final_deduced_category_slug = Some(slug.clone());
                    println!(
                        "[analyze_archive]   -> Matched category from filename via exact cleaned name: {} -> {}",
                        cleaned_stem,
                        slug
                    );
                } else {
                    // Prio 3: Known name contains cleaned stem part
                    let stem_words: Vec<&str> = cleaned_stem
                        .split_whitespace()
                        .filter(|w| w.len() > 2)
                        .collect();
                        ...
                        'cat_loop: for (
                            ...
                            for word in &stem_words {
                                if cat_name_lower.contains(word) {
                                    final_deduced_category_slug = Some(cat_slug.clone());
                                    println!(
                                        "[analyze_archive]   -> Matched category from filename via word contains: '{}' contains '{}' -> {}",
                                        cat_name_lower,
                                        word,
                                        cat_slug
                                    );
        ...
        println!(
            "[analyze_archive] Skipping filename deduction (already found entity and category)."
        );
    }
    ...
    if deduced_mod_name.is_none() || deduced_mod_name.as_deref() == Some("") {
        deduced_mod_name = file_path
            .file_stem()
            .and_then(OsStr::to_str)
            .map(|s| clean_and_extract_name(s)); // Use cleaner here too
        println!(
            "[analyze_archive] Used archive filename for deduced name: {:?}",
            deduced_mod_name
        );
    }
    ...
    if let Some(name) = &deduced_mod_name {
        ...
        if !cleaned.is_empty() {
            ...
            println!(
                "[analyze_archive] Warning: Name cleanup resulted in empty string, using raw file stem: {:?}",
                deduced_mod_name
            );
        }
    }
    ...
    // --- Final Log ---
    println!(
        "[analyze_archive] Final Deductions: Name={:?}, Author={:?}, Category={:?}, Entity={:?}, Preview={:?}, RawINI Target={:?}, RawINI Type={:?}",
        deduced_mod_name,
        deduced_author,
        final_deduced_category_slug,
        final_deduced_entity_slug,
        detected_preview_internal_path,
        raw_ini_target_found,
        raw_ini_type_found
    );
...
#[command]
fn read_archive_file_content(
    archive_path_str: String,
    internal_file_path: String
) -> CmdResult<Vec<u8>> {
    println!(
        "[read_archive_file_content] Reading '{}' from archive '{}'",
        internal_file_path,
        archive_path_str
    );
    ...
    match extension.as_deref() {
        Some("zip") => {
            let file = fs::File
                ::open(&archive_path)
                .map_err(|e| format!("Zip Read: Failed open: {}", e))?;
            let mut archive = ZipArchive::new(file).map_err(|e|
                format!("Zip Read: Failed read archive: {}", e)
            )?;
            ...
            let result = match archive.by_name(&internal_path_normalized) {
                ...
                Err(ZipError::FileNotFound) =>
                    Err(format!("Zip Read: Internal file '{}' not found.", internal_file_path)),
                Err(e) =>
                    Err(
                        format!(
                            "Zip Read: Error accessing internal file '{}': {}",
                            internal_file_path,
                            e
                        )
                    ),
        ...
        Some("7z") => {
            ...
            let mut archive = sevenz_rust::SevenZReader
                ::open(&archive_path_str, Password::empty())
                .map_err(|e| format!("7z Read: Failed open: {}", e))?;

            archive
                .for_each_entries(|entry, reader| {
                    if found_content.is_some() || found_error.is_some() {
                        return Ok(false);
                    }
                    ...
                    if entry_name_normalized == internal_path_normalized {
                        let mut content_bytes = Vec::new();
                        let mut buffer = [0u8; 4096];
                        let read_result: Result<(), io::Error> = (|| {
                            // Use closure for ? propagation
                            loop {
                                let bytes_read = reader.read(&mut buffer)?;
                                if bytes_read == 0 {
                                    break;
                        ...
                        match read_result {
                            Ok(()) => {
                                found_content = Some(content_bytes);
                            }
                            Err(e) => {
                                found_error = Some(
                                    format!(
                                        "7z Read: Error reading content '{}': {}",
                                        internal_file_path,
                                        e
                                    )
                                );
                ...
                .map_err(|e: sevenz_rust::Error|
                    format!("7z Read: Error iterating entries: {}", e)
                )?;

            if let Some(content) = found_content {
                Ok(content)
            } else if let Some(err) = found_error {
                Err(err)
            } else {
                Err(format!("7z Read: Internal file '{}' not found.", internal_file_path))
            }
        }
        Some("rar") => {
            ...
            loop {
                match archive.read_header() {
                    Ok(Some(header_state)) => {
                        // Returns OpenArchive<..., CursorBeforeFile>
                        let entry_filename = &header_state.entry().filename; // Access header via entry()
                        let entry_name_normalized = entry_filename
                            .to_string_lossy()
                            .replace("\\", "/");
                        ...
                        if entry_name_normalized == internal_path_normalized {
                            ...
                            match header_state.read() {
                                ...
                                Err(e) => {
                                    ...
                                    return Err(
                                        format!(
                                            "Rar Read: Error reading content '{}': {}",
                                            internal_file_path,
                                            e
                                        )
                                    );
                    ...
                    Err(e) => {
                        return Err(format!("Rar Read: Error reading header: {}", e));
                    }
                }
            }
            found_content.ok_or_else(||
                format!("Rar Read: Internal file '{}' not found.", internal_file_path)
            )
        }
...
fn import_archive(
    ...
    println!(
        "[import_archive] Importing '{}', internal path '{}' for entity '{}'. Image Data Provided: {}. Add to presets: {:?}",
        archive_path_str,
        if selected_internal_root.is_empty() {
            "(Extract All)"
        } else {
            &selected_internal_root
        }, // Indicate if extracting all
        target_entity_slug,
        image_data.is_some(),
        preset_ids
    );
    ...
    if target_entity_slug.trim().is_empty() {
        return Err("Target Entity must be selected.".to_string());
    }
    let archive_path = PathBuf::from(&archive_path_str);
    if !archive_path.is_file() {
        return Err(format!("Archive file not found: {}", archive_path.display()));
    ...
    let (target_category_slug, target_entity_id): (String, i64) = conn_guard
        .query_row(
        ...
        .map_err(|e| {
            match e {
                rusqlite::Error::QueryReturnedNoRows =>
                    format!("Target entity '{}' not found.", target_entity_slug),
                _ => format!("DB Error get target entity: {}", e),
            }
        })?;

    let target_mod_folder_name = mod_name
        .trim()
        .replace(" ", "_")
        .replace(".", "_")
        .replace("'", "")
        .replace("\"", "");
    if target_mod_folder_name.is_empty() {
        return Err("Mod Name results in invalid folder name.".to_string());
    }
    ...
    fs
        ::create_dir_all(&final_mod_dest_path)
        .map_err(|e|
            format!("Failed create dest directory '{}': {}", final_mod_dest_path.display(), e)
        )?;
    println!(
        "[import_archive] Target destination folder created/ensured: {}",
        final_mod_dest_path.display()
    );

    let tx = conn_guard
        .transaction()
        .map_err(|e| format!("Failed start import transaction: {}", e))?;
    ...
    let prefix_to_extract = prefix_to_extract_norm
        .strip_suffix('/')
        .unwrap_or(&prefix_to_extract_norm);
    ...
    let extraction_result: Result<usize, String> = (|| {
        match extension.as_deref() {
            Some("zip") => {
                let file = fs::File
                    ::open(&archive_path)
                    .map_err(|e| format!("Zip Extract: Failed open: {}", e))?;
                let mut archive = ZipArchive::new(file).map_err(|e|
                    format!("Zip Extract: Failed read archive: {}", e)
                )?;
                for i in 0..archive.len() {
                    let mut file_in_zip = archive
                        .by_index(i)
                        .map_err(|e| format!("Zip Extract: Failed read entry #{}: {}", i, e))?;
                    let internal_path_obj_opt = file_in_zip
                        .enclosed_name()
                        .map(|p| p.to_path_buf());
                    if internal_path_obj_opt.is_none() {
                        continue;
                    }
                    ...
                    if file_in_zip.is_dir() {
                        ...
                        if let Some(p) = outpath.parent() {
                            if !p.exists() {
                                fs
                                    ...
                                    .map_err(|e|
                                        format!(
                                            "Zip Extract: Failed create parent '{}': {}",
                                            p.display(),
                                            e
                                        )
                                    )?;
                        ...
                        let mut outfile = fs::File
                            ...
                            .map_err(|e|
                                format!(
                                    "Zip Extract: Failed create file '{}': {}",
                                    outpath.display(),
                                    e
                                )
                            )?;
                        std::io
                            ...
                            .map_err(|e|
                                format!(
                                    "Zip Extract: Failed copy content '{}': {}",
                                    outpath.display(),
                                    e
                                )
                            )?;
            ...
            Some("7z") => {
                ...
                archive
                    ...
                    .map_err(|e: sevenz_rust::Error|
                        format!("7z Extract: Error processing entries: {}", e)
                    )?;
            }
            Some("rar") => {
                let mut archive = Archive::new(&archive_path_str)
                ...
                loop {
                    match archive.read_header().map_err(|e| e.to_string())? {
                        Some(header_state) => {
                            ...
                            let internal_path_str = entry_filename
                                .to_string_lossy()
                                .replace("\\", "/")
                                .to_string();
                            let internal_path_obj = PathBuf::from(&internal_path_str);
                                ...
                                if let Some(p) = outpath.parent() {
                                    if !p.exists() {
                                        fs
                                            ...
                                            .map_err(|e|
                                                format!(
                                                    "Rar Extract: Failed create parent '{}': {}",
                                                    p.display(),
                                                    e
                                                )
                                            )?;
            ...
            _ => {
                return Err(format!("Unsupported archive type for extraction: {:?}", extension));
            }
        }
    ...
    if let Some(data) = image_data {
        ...
        match fs::write(&target_image_path, data) {
            Ok(_) => {
                println!(
                    "[import_archive] Image data written successfully to '{}'.",
                    target_image_path.display()
                );
            ...
            Err(e) => {
                eprintln!(
                    "[import_archive] ERROR: Failed to save pasted image data to '{}': {}. Preview will be missing.",
                    target_image_path.display(),
                    e
                );
            }
        ...
        if source_path.is_file() {
            ...
            match fs::copy(&source_path, &target_image_path) {
                Ok(_) => {
                    println!(
                        "[import_archive] Image file copied successfully to '{}'.",
                        target_image_path.display()
                    );
                    image_filename_for_db = Some(TARGET_IMAGE_FILENAME.to_string());
                }
                Err(e) => {
                    eprintln!(
                        "[import_archive] ERROR: Failed copy user preview to '{}': {}. Preview will be missing.",
                        target_image_path.display(),
                        e
                    );
                }
    ...
    println!(
        "[import_archive] Image handling complete. Filename to save in DB: {:?}",
    ...
    let check_existing: Option<i64> = tx
        .query_row(
            "SELECT id FROM assets WHERE entity_id = ?1 AND folder_name = ?2",
        ...
        .optional()
        .map_err(|e|
            format!("DB error check existing import '{}': {}", relative_path_for_db_str, e)
        )?;

    if check_existing.is_some() {
        fs::remove_dir_all(&final_mod_dest_path).ok();
        return Err(
            format!("Database entry already exists for '{}'. Aborting.", relative_path_for_db_str)
        );
    }

    println!(
        "[import_archive] Adding asset to DB: entity_id={}, name={}, path={}, image={:?}",
        target_entity_id,
        mod_name,
        relative_path_for_db_str,
        image_filename_for_db
    );
    tx
        .execute(
    ...
    if let Some(ids) = preset_ids {
        if !ids.is_empty() {
            ...
            for preset_id in ids {
                insert_preset_stmt
                    .execute(params![preset_id, new_asset_id, 1]) // Default to enabled state 1 when importing
                    .map_err(|e|
                        format!(
                            "Failed insert new asset {} into preset {}: {}",
                            new_asset_id,
                            preset_id,
                            e
                        )
...
fn create_preset(name: String, db_state: State<DbState>) -> CmdResult<Preset> {
    ...
    let preset_id = {
        ...
        // Insert new preset
        tx
            .execute("INSERT INTO presets (name) VALUES (?1)", params![name])
        ...
        // Use another block scope for the statement and iteration
        {
            ...
            let asset_iter_result = stmt.query_map([], |row| {
                Ok((row.get::<_, i64>(0)?, row.get::<_, String>(1)?.replace("\\", "/")))
            });
                    ...
                    for asset_result in asset_iter {
                        match asset_result {
                            Ok((asset_id, clean_relative_path_str)) => {
                                let clean_relative_path = PathBuf::from(&clean_relative_path_str);
                                let filename_osstr = clean_relative_path
                                    .file_name()
                                    .unwrap_or_default();
                                let filename_str = filename_osstr.to_string_lossy();
                                if filename_str.is_empty() {
                                ...
                                let is_currently_enabled = if full_path_if_enabled.is_dir() {
                                    1
                                } else if full_path_if_disabled.is_dir() {
                                    0
                                } else {
                                    println!(
                                        "[create_preset] Warning: Asset ID {} folder not found on disk during preset save (path: {}). Skipping.",
                                        asset_id,
                                        clean_relative_path_str
                                    );
                                    continue;
                                ...
                                tx
                                    ...
                                    .map_err(|e|
                                        format!(
                                            "Failed to save state for asset {}: {}",
                                            asset_id,
                                            e
                                        )
                                    )?;
                            ...
                            Err(e) => {
                                return Err(format!("Error fetching asset row: {}", e));
                            } // Rollbacks on return
                        }
                    }
                }
                Err(e) => {
                    return Err(format!("Error preparing asset iterator: {}", e));
                } // Rollbacks on return
            }
...
fn get_presets(db_state: State<DbState>) -> CmdResult<Vec<Preset>> {
    let conn = db_state.0.lock().map_err(|_| "DB lock poisoned".to_string())?;
    let mut stmt = conn
        .prepare("SELECT id, name, is_favorite FROM presets ORDER BY name ASC")
        .map_err(|e| e.to_string())?;
...
async fn apply_preset(
    ...
    // --- Get base path first ---
    let base_mods_path = get_mods_base_path_from_settings(&db_state).map_err(|e|
        format!("Cannot apply preset: {}", e)
    )?;

    // --- Fetch preset assets ---
    let preset_assets_to_apply = {
        // Use block scope for connection lock
        let conn = db_state.0.lock().map_err(|_| "DB lock poisoned".to_string())?;
        let mut stmt = conn
            .prepare(
        ...
        match preset_assets_iter_result {
            Ok(iter) =>
                iter
                    .collect::<SqlResult<Vec<(i64, bool, String, String)>>>() // Include name
                    .map_err(|e| format!("Failed to collect preset assets: {}", e))?,
            Err(e) => {
                return Err(format!("Error preparing preset asset iterator: {}", e));
            }
        }
    ...
    for (
        ...
        if filename_str.is_empty() {
            let err_msg = format!(
                "Skipping asset ID {}: Invalid folder name '{}'.",
                asset_id,
                clean_relative_path_str
            );
        ...
        let construct_full_path = |name: &str| -> PathBuf {
            match relative_parent_path {
                Some(parent) if parent.as_os_str().len() > 0 =>
                    base_mods_path.join(parent).join(name),
        ...
        if full_path_if_enabled.is_dir() {
            ...
            let err_msg = format!(
                "Skipping asset '{}' (ID {}): Folder not found on disk (path: '{}').",
                asset_name,
                asset_id,
                clean_relative_path_str
            );
        ...
        if current_is_enabled != desired_is_enabled {
            ...
            println!(
                "[apply_preset] Renaming '{}' -> '{}' (Desired Enabled: {})",
                source_path.display(),
                target_path.display(),
                desired_is_enabled
            );
...
fn toggle_preset_favorite(
    ...
    conn
        .execute("UPDATE presets SET is_favorite = ?1 WHERE id = ?2", params![fav_value, preset_id])
...
fn delete_preset(preset_id: i64, db_state: State<DbState>) -> CmdResult<()> {
    ...
    let changes = conn
        .execute("DELETE FROM presets WHERE id = ?1", params![preset_id])
...
fn get_dashboard_stats(db_state: State<DbState>) -> CmdResult<DashboardStats> {
    ...
    for row_result in cat_rows {
        match row_result {
            Ok((name, count)) => {
                category_counts.insert(name, count);
            }
            Err(e) => {
                eprintln!("[get_dashboard_stats] Error processing category count row: {}", e);
            }
        }
    }
    ...
    let mut asset_folders_stmt = conn
        .prepare("SELECT folder_name FROM assets")
        .map_err(|e| format!("Failed to prepare asset folder fetch: {}", e))?;
    let asset_folder_rows = asset_folders_stmt
    ...
    for folder_result in asset_folder_rows {
        match folder_result {
            ...
            Err(e) => {
                eprintln!("[get_dashboard_stats] Error fetching asset folder row: {}", e);
            }
        }
    }
...
#[command]
fn get_entities_by_category_with_counts(
    category_slug: String,
    db_state: State<DbState>
) -> CmdResult<Vec<EntityWithCounts>> {
    ...
    let entity_rows_iter = entity_stmt
        .query_map(params![category_id], |row| {
            Ok((
                row.get::<_, i64>(0)?,
                row.get::<_, i64>(1)?,
    ...
    let mut asset_folder_stmt = conn
        .prepare("SELECT folder_name FROM assets WHERE entity_id = ?1")
    ...
    for entity_result in entity_rows_iter {
        match entity_result {
            Ok((id, cat_id, name, slug, details, base_image)) => {
                ...
                let asset_folder_rows_result = asset_folder_stmt.query_map(params![id], |row|
                    row.get::<_, String>(0)
                );
                ...
                match asset_folder_rows_result {
                        ...
                        for folder_result in rows {
                            match folder_result {
                                Ok(clean_relative_path_str) => {
                                    ...
                                    let clean_relative_path = PathBuf::from(
                                        clean_relative_path_str.replace("\\", "/")
                                    );
                                    let filename_osstr = clean_relative_path
                                        .file_name()
                                        .unwrap_or_default();
                                ...
                                Err(e) =>
                                    eprintln!(
                                        "[get_entities_with_counts] Error fetching asset folder row for entity {}: {}",
                                        id,
                                        e
                                    ),
                            }
                    ...
                    Err(e) =>
                        eprintln!(
                            "[get_entities_with_counts] Error querying asset folders for entity {}: {}",
                            id,
                            e
                        ),
                }
    ...
    println!(
        "[get_entities_with_counts] Found {} entities with counts for category '{}'",
        results.len(),
        category_slug
    );
    Ok(results)
...
fn overwrite_preset(preset_id: i64, db_state: State<DbState>) -> CmdResult<()> {
    ...
    let base_mods_path = get_mods_base_path_from_settings(&db_state).map_err(|e|
        format!("Cannot overwrite preset (failed to get mods path): {}", e)
    )?;
    ...
    let delete_count = tx
        .execute("DELETE FROM preset_assets WHERE preset_id = ?1", params![preset_id])
        ...
        for (asset_id, clean_relative_path_str) in assets_to_save {
            ...
            let is_currently_enabled_on_disk = if full_path_if_enabled.is_dir() {
                ...
                if full_path_if_disabled.is_dir() {
                    ...
                    println!(
                        "[overwrite_preset] Warning: Asset ID {} folder not found on disk during preset save (path: {}). Skipping.",
                        asset_id,
                        clean_relative_path_str
                    );
                    not_found_count += 1;
            ...
            insert_stmt
                .execute(params![preset_id, asset_id, is_currently_enabled_on_disk])
    ...
    println!(
        "[overwrite_preset] Preset ID {} overwritten successfully. Saved state for {} assets (skipped {} not found).",
        preset_id,
        saved_count,
        not_found_count
    );
...
#[command]
fn get_ini_keybinds(asset_id: i64, db_state: State<DbState>) -> Result<Vec<KeybindInfo>, String> {
    // CmdResult is Result<T, String>
    ...
    let result: Result<Vec<KeybindInfo>, String> = (|| {
        ...
        let mods_folder_path_str_opt: Option<String> = conn
            .query_row(
            ...
            .optional()
            .map_err(|e| format!("DB error fetching mods folder setting: {}", e))?;
        // --- End Fix ---

        let base_mods_path = mods_folder_path_str_opt.map(PathBuf::from).ok_or_else(|| {
            eprintln!("[get_ini_keybinds] ERROR: Mods folder path not set in settings.");
            "Mods folder path not set".to_string()
        })?;
        println!(
            "[get_ini_keybinds] Base mods path obtained directly: {}",
            base_mods_path.display()
        );

        println!("[get_ini_keybinds] Calling find_asset_ini_paths...");
        let ini_paths = find_asset_ini_paths(conn, asset_id, &base_mods_path).map_err(|e| {
        ...
        for ini_path in ini_paths {
            ...
            let file = match File::open(&ini_path) {
                ...
                Err(e) => {
                    eprintln!(
                        "[get_ini_keybinds] ERROR: Failed to open INI file {}: {}. Skipping.",
                        ini_path.display(),
                        e
                    );
            ...
            for line_result in reader.lines() {
                let line_raw = match line_result {
                    ...
                    Err(_) => {
                        println!(
                            "[get_ini_keybinds] Warning: Skipping unreadable line in {}",
                            ini_path.display()
                        );
                        continue;
                    }
                ...
                if
                    !found_constants_tag &&
                    line.starts_with(';') &&
                    line[1..].trim_start().to_lowercase().contains("constants")
                {
                    println!(
                        "[get_ini_keybinds] Found '; Constants' marker in {}",
                        ini_path.display()
                    );
                ...
                if found_constants_tag {
                    if line.starts_with('[') && line.ends_with(']') {
                        ...
                        line.to_lowercase().starts_with("key") &&
                        line.contains('=')
                    {
            ...
            if !found_keybinds.is_empty() {
                println!(
                    "[get_ini_keybinds] SUCCESS: Found {} keybinds (after Constants) in {}. Returning.",
                    found_keybinds.len(),
                    ini_path.display()
                );
                ...
                println!(
                    "[get_ini_keybinds] No keybinds found (after Constants) in {}. Checking next file.",
                    ini_path.display()
                );
            }
...
fn open_asset_folder(asset_id: i64, db_state: State<DbState>) -> CmdResult<()> {
    ...
    let result = (|| {
        ...
        let asset_info = get_asset_location_info(conn, asset_id).map_err(|e|
            format!("Failed to get asset info for opening folder: {}", e)
        )?;
        ...
        let relative_path_buf = PathBuf::from(
            &asset_info.clean_relative_path.replace("/", std::path::MAIN_SEPARATOR_STR)
        ); // Ensure DB path uses native sep before join? Less critical usually.
        ...
        match mod_folder_path_on_disk {
            Some(mod_path) => {
                ...
                if cfg!(target_os = "windows") {
                    ...
                    arg = mod_path
                        .to_str()
                        .ok_or("Invalid UTF-8 path string for macOS")?
                        .to_string();
                } else {
                    command_name = "xdg-open";
                    arg = mod_path
                        .to_str()
                        .ok_or("Invalid UTF-8 path string for Linux")?
                        .to_string();
                }
                // --- End Fix ---
                ...
                match Command::new(command_name).args(&[arg]).spawn() {
                    ...
                    Err(e) => {
                        eprintln!(
                            "Failed to spawn file explorer command '{}': {}",
                            command_name,
                            e
                        );
                        Err(format!("Failed to open folder using '{}': {}", command_name, e))
...
fn add_asset_to_presets(
    ...
    println!(
        "[add_asset_to_presets] Adding/Updating asset ID {} in presets: {:?}",
        asset_id,
        preset_ids
    );

    let base_mods_path = get_mods_base_path_from_settings(&db_state).map_err(|e|
        format!("Cannot add/update presets (failed to get mods path): {}", e)
    )?;
    ...
    let current_is_enabled = match get_current_asset_enabled_state(&tx, asset_id, &base_mods_path) {
        ...
        Err(e) => {
            eprintln!(
                "[add_asset_to_presets] Error getting current state for asset {}: {}. Aborting.",
                asset_id,
                e
            );
            return Err(
                format!("Failed to determine current enabled state for asset {}: {}", asset_id, e)
            );
        }
    };

    println!(
        "[add_asset_to_presets] Determined current enabled state for asset {} as: {}",
        asset_id,
        current_is_enabled
    );
    ...
    let mut changes_made = 0; // Tracks total rows affected (inserts + updates)
    {
        ...
        for preset_id in preset_ids {
            let changes = upsert_stmt
                .execute(params![preset_id, asset_id, current_is_enabled])
                .map_err(|e|
                    format!("Failed to upsert asset {} into preset {}: {}", asset_id, preset_id, e)
                )?;
    ...
    println!(
        "[add_asset_to_presets] Successfully added/updated asset {} in presets. Rows affected: {}",
        asset_id,
        changes_made
    );
    Ok(())
...
fn get_available_games(app_handle: AppHandle) -> CmdResult<Vec<String>> {
    ...
    let mut games: HashSet<String> = PREDEFINED_GAMES.iter()
        .map(|&s| s.to_string())
        .collect();
                    ...
                    if let Ok(entry) = entry_result {
                        ...
                        if path.is_file() {
                            if let Some(filename_str) = path.file_name().and_then(|n| n.to_str()) {
                                ...
                                if
                                    filename_str.starts_with(DB_FILENAME_PREFIX) &&
                                    filename_str.ends_with(".sqlite")
                                {
...
fn switch_game(app_handle: AppHandle, target_game_slug: String) -> CmdResult<String> {
    // Keep AppHandle for potential future use, though not needed for exit
    ...
    Ok(
        format!(
            "Successfully configured to switch to '{}' on next launch. Please close and restart the application.",
            target_game_slug.to_uppercase()
        )
    )
}
...
fn main() {
    ...
    tauri::Builder
        ...
        .setup(|app| {
            ...
            if !data_dir.exists() {
                println!(
                    "App data directory does not exist, attempting to create: {}",
                    data_dir.display()
                );
                if let Err(e) = fs::create_dir_all(&data_dir) {
                    // If creation fails (permissions?), it's fatal.
                    eprintln!(
                        "FATAL: Failed to create app data directory at {}: {}",
                        data_dir.display(),
                        e
                    );
                    dialog::blocking::message(
            ...
            if last_slug != requested_slug {
                ...
                let last_game_archive_path = data_dir.join(
                    format!("{}{}.sqlite", DB_FILENAME_PREFIX, last_slug)
                );
                let requested_game_archive_path = data_dir.join(
                    format!("{}{}.sqlite", DB_FILENAME_PREFIX, requested_slug)
                );
                ...
                if active_db_path.exists() {
                    println!(
                        ...
                        ACTIVE_DB_FILENAME,
                        last_slug,
                        last_game_archive_path.display()
                    );
                    ...
                    println!(
                        "Warning: {} not found, cannot archive game '{}'.",
                        ACTIVE_DB_FILENAME,
                        last_slug
                    );
                }
                ...
                if requested_game_archive_path.exists() {
                    println!(
                        "Activating '{}' from '{}'",
                        ACTIVE_DB_FILENAME,
                        requested_game_archive_path.display()
                    );
                    ...
                    if let Err(e) = fs::rename(&requested_game_archive_path, &active_db_path) {
                        ...
                        if last_game_archive_path.exists() {
                            println!(
                                "Attempting rollback: Renaming {} back to {}",
                                last_game_archive_path.display(),
                    ...
                    println!(
                        "Archive for requested game '{}' ('{}') not found. New DB will be created.",
                        requested_slug,
                        requested_game_archive_path.display()
                    );
                }
                ...
                let active_db_path = get_app_data_dir(&app_handle)
                    .expect("Data dir checked previously")
                    .join(ACTIVE_DB_FILENAME);
                if !active_db_path.exists() {
                    println!(
                        "Warning: Config indicates no switch needed, but '{}' does not exist. A new DB will be created for '{}'.",
                        ACTIVE_DB_FILENAME,
                        requested_slug
                    );
                }
            ...
            match
                get_setting_value(
                    &db_state.0.lock().expect("DB lock poisoned during setup check"),
                    SETTINGS_KEY_MODS_FOLDER
                )
            {
                // Lock mutex to access connection.
        ...
        .invoke_handler(
            generate_handler![
                ...
                toggle_asset_enabled,
                get_asset_image_path,
                run_traveler_migration,
                open_mods_folder,
                // Scan & Count
                scan_mods_directory,
